...
const tiers = [
  {
    ...
    services: [
      { icon: BrainCircuit, title: "AI & Business Automation", desc: "Eliminate manual data entry, document processing, and repetitive workflows with practical AI. Built into your existing systems — not bolted on top.", highlight: "Most clients see ROI within 90 days.", accent: "#1FB5C9" },
      { icon: LayoutDashboard, title: "Operational Dashboards", desc: "Real-time command centers for your team — pipeline velocity, KPIs, system health — surfaced in one operational view your leadership can act on.", accent: "#D4A574" },
      { icon: Network, title: "Digital Transformation", desc: "Migrate legacy operations to modern, scalable infrastructure. We handle strategy, build, and transition without disrupting your current operations.", accent: "#4D8B72" },
    ]
  },
  {
    name: "Automate",
    ...
    services: [
      { icon: ShoppingCart, title: "Ecommerce Development", desc: "Custom online stores built for conversion — with automated inventory, seamless payment integrations, and experiences your customers will remember.", accent: "#E8745B" },
      { icon: Workflow, title: "CRM & Business Systems", desc: "Replace disconnected spreadsheets with a unified pipeline. Track leads, manage clients, and close deals from one system built for how you actually work.", accent: "#D4A574" },
      { icon: Users, title: "Client Portals", desc: "Branded, secure environments where clients submit documents, track projects, and communicate — cutting out the endless email chains.", accent: "#1FB5C9" },
    ]
  },
  {
    name: "Launch",
    ...
    services: [
      { icon: Globe, title: "Professional Website", desc: "A fast, modern website that represents your business and converts visitors. Built to rank on Google, load instantly, and look great on every device. Starting point for any business.", accent: "#4D8B72" },
      { icon: Smartphone, title: "Landing Pages & Funnels", desc: "Single-purpose pages designed to capture leads, sell products, or launch something new. Clear, fast, conversion-optimized.", accent: "#E8745B" },
      { icon: MonitorSmartphone, title: "Branding & Digital Presence", desc: "Logo, brand identity, and digital setup — everything a new business needs to look credible and professional from day one.", accent: "#D4A574" },
    ]
  },
];
    ...
    <section id="services" className="py-32 relative bg-[#0B1020] z-10">
      <div className="max-w-7xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-20 max-w-3xl">
          <div className="text-sm font-mono tracking-widest text-[#98A3B3] uppercase mb-4">Three Starting Points</div>
          <h2 className="text-4xl md:text-5xl font-display font-medium text-white mb-5 tracking-tight text-balance">
            Every project builds on<br/>the same foundation.
          </h2>
          <p className="text-lg text-[#B8C2CE] font-light leading-relaxed">
            Launch, Automate, or Transform — pick your starting point, and the systems we build stay compatible as you move to the next stage.
          </p>
        </motion.div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {tier.services.map((svc, si) => (
                  <motion.div key={si} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }}
                    ...
                    className="group relative p-6 rounded-xl bg-[#0B1020]/60 border border-white/[0.04] hover:border-[var(--accent)]/30 transition-all duration-400">
                    <div className="w-9 h-9 rounded-lg bg-[#12182B] border border-white/[0.05] flex items-center justify-center mb-4 transition-colors duration-300"
                    ...
                    {svc.highlight && (
                      <div className="mt-3 text-[10px] font-mono uppercase tracking-widest flex items-center gap-1.5" style={{ color: svc.accent }}>
                        <span className="w-1.5 h-1.5 rounded-full" style={{ background: svc.accent }} />{svc.highlight}
                      </div>
                    )}
                  </motion.div>
                ))}
