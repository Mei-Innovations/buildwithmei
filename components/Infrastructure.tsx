      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col lg:flex-row items-center gap-16">
          ...
          <motion.div initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="flex-1">
            <div className="text-sm font-mono tracking-widest text-[#555] uppercase mb-4">Systems Engineering</div>
            ...
            <div className="space-y-8">
              {[
                ...
                >
                  <div className="w-10 h-10 rounded-md bg-[#12182B] border border-white/8 flex items-center justify-center text-[#555] group-hover:text-brand-emerald transition-colors duration-300 mt-1 shrink-0">
                    <feat.icon size={20} strokeWidth={1.5} />
                  </div>
                  <div>
                    <h4 className="text-white font-medium mb-1 tracking-tight">{feat.title}</h4>
                    <p className="text-sm text-[#555] font-light leading-relaxed">{feat.desc}</p>
                  </div>
                </motion.div>
              ))}
            </div>
              ...
              <div className="rounded-lg overflow-hidden bg-[#0D111A] border border-white/[0.04]">
                <div className="h-14 border-b border-white/[0.04] flex items-center px-5 justify-between bg-[#12182B]">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-[10px] font-mono tracking-widest text-[#444]">operations_dashboard.tsx</div>
                  </div>
                ...
                <div className="p-6">
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    {[
                      ...
                      >
                        <div className="text-[9px] text-[#444] font-mono tracking-widest mb-3">{m.label}</div>
                        <div className="flex justify-between items-end">
                          ...
                          {m.badge && <span className="text-[9px] font-mono text-brand-emerald bg-brand-emerald/10 px-1.5 py-0.5 rounded border border-brand-emerald/20">{m.badge}</span>}
                          {m.chart && (
                  ...
                  <div className="border border-white/[0.03] rounded-lg overflow-hidden bg-[#0B1020]">
                    <div className="px-5 py-2.5 bg-[#0D111A] border-b border-white/[0.03] text-[9px] text-[#444] font-mono flex justify-between tracking-widest uppercase">
                    ...
                    <div className="p-5 space-y-4 font-mono text-xs">
                      {[
                        ...
                        <div key={i} className={`flex justify-between items-center ${row.dim ? "opacity-40" : "group"}`}>
                          <span className={`flex items-center gap-3 text-[#888] ${!row.dim ? "group-hover:text-white" : ""} transition-colors`}>
                            <span className={`${row.color} ${row.spin ? "animate-spin inline-block" : ""}`}>{row.icon}</span>
                            {row.name}
                          </span>
                          <span className={`${row.dim ? "text-[#333]" : "text-[#444]"} ${row.pulse ? "animate-pulse" : ""}`}>{row.status}</span>
                        </div>
                      ))}
                    </div>
                  </div>
