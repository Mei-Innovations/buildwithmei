        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {ecosystems.map((eco, i) => (
            <motion.div key={eco.id} id={eco.id}
              ...
              style={{ ['--accent' as string]: eco.accent }}
              className="group relative p-8 rounded-xl bg-[#12182B] border border-white/5 hover:border-[var(--accent)]/30 transition-all duration-400 flex flex-col scroll-mt-28">
              ...
              <eco.icon className="absolute -right-8 -top-8 w-36 h-36 opacity-[0.05] group-hover:opacity-[0.09] transition-opacity duration-700 pointer-events-none" style={{ color: eco.accent }} strokeWidth={1} />
              <div className="relative z-10 flex flex-col h-full">
                {/* Industry environment — photography with dark overlay (untouched) */}
                <div className="relative h-36 -mx-8 -mt-8 mb-7 overflow-hidden rounded-t-xl">
                  ...
                  <div className="absolute top-0 left-0 right-0 h-[3px]" style={{ background: eco.accent }} />
                </div>
                <div className="flex items-start justify-between mb-6">
                  <div className="w-10 h-10 rounded-lg bg-[#0B1020] border border-white/8 flex items-center justify-center transition-colors duration-300" style={{ color: eco.accent }}>
                    <eco.icon size={20} strokeWidth={1.5} />
                  </div>
                </div>
                <h3 className="text-lg font-medium text-white mb-1.5 tracking-tight">{eco.title}</h3>
                <div className="text-[10px] font-mono tracking-widest uppercase mb-6" style={{ color: eco.accent, opacity: 0.85 }}>{eco.audience}</div>
                ...
                <div className="mb-5">
                  <div className="text-[10px] font-mono tracking-widest mb-2.5 uppercase" style={{ color: eco.accent }}>MEI Intelligence Layer</div>
                ...
                <div className="mt-auto pt-4 border-t border-white/5">
                  <a href={SITE.calendly} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-sm hover:text-white transition-colors font-medium" style={{ color: eco.accent }}>
